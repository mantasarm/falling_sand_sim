...
pub fn debug_ui(ctx: &Context, app: &App, debug_info: &mut DebugInfo, chunk_manager: &mut ChunkManager) {
...
pub fn debug_render(ctx: &Context, debug_info: &mut DebugInfo, chunk_manager: &ChunkManager) {
    Window::new("Debug window").resizable(false).collapsible(true).title_bar(true).open(&mut debug_info.debug_window).show(ctx, |ui| {
        ...
        ui.label("Cell {");
        ui.label(format!("    element: {:?} ", chunk_manager.hovering_cell.element));
        ui.label(format!("    action: {:?} ", chunk_manager.hovering_cell.action));
        ui.label(format!("    state: {:?}", chunk_manager.hovering_cell.state));
        ui.label(format!("    velocity: Vec2({:.2}, {:.2})", chunk_manager.hovering_cell.velocity.x, chunk_manager.hovering_cell.velocity.y));
        ui.label(format!("    density: {:?}", chunk_manager.hovering_cell.density));
        ui.label(format!("    drag: {:?}", chunk_manager.hovering_cell.drag));
        ui.label(format!("    color: {:?}", chunk_manager.hovering_cell.color));
        ui.label(format!("    lifetime: {:?}", chunk_manager.hovering_cell.lifetime));
        ui.label("}");
    });
}
...
pub fn debug_mem_usage(ctx: &Context, debug_info: &mut DebugInfo) {
    ...
    Window::new("Mem usage").resizable(false).collapsible(true).open(&mut debug_info.debug_mem_usage).show(ctx, |ui| {
...
pub fn debug_sky_editor(ctx: &Context, debug_info: &mut DebugInfo) {
    Window::new("Sky color").resizable(false).collapsible(true).open(&mut debug_info.sky_editor).show(ctx, |ui| {
        ui.add(Slider::new(&mut debug_info.sky_color[0], 0..=255).clamp_to_range(true).prefix("r: "));
        ui.add(Slider::new(&mut debug_info.sky_color[1], 0..=255).clamp_to_range(true).prefix("g: "));
        ui.add(Slider::new(&mut debug_info.sky_color[2], 0..=255).clamp_to_range(true).prefix("b: "));
    });
}
