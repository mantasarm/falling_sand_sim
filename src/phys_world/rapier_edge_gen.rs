...
use super::{chunk::{Chunk, COLS, ROWS, UPSCALE_FACTOR}, element::ElColliderType, rapier_world_handler::PHYS_SCALE};

pub fn edges_from_chunk(chunk: &mut Chunk) {
	let mut map = vec![];
	for i in 0..ROWS {
		for j in 0..COLS {
			if chunk.grid[j][i].collider_type == ElColliderType::Chunk {
				map.push(1.);
			} else {
				map.push(0.);
			}
